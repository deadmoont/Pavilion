    ...
    return Scaffold(
      appBar: AppBar(
        ...
        backgroundColor: const Color(
            0xFF121212), // Set the background color of the AppBar to black
      ),
      backgroundColor: const Color(
          0xFF121212), // Set the background color of the scaffold to black
      body: Padding(
        padding: const EdgeInsets.symmetric(
            horizontal: 4.0,
            vertical: 2.0), // Add horizontal and vertical padding
        child: isLoading
            ? const Center(
                child:
            ...
            : images.isEmpty
                ? const Center(
                    child: Text(
                        'No images found.')) // Handle case where there are no images
                : Padding(
                    padding: const EdgeInsets.symmetric(
                        horizontal:
                            20.0), // Add horizontal padding around the photo list
                    child: MasonryGridView.builder(
                      ...
                      itemBuilder: (context, index) {
                        return ClipRRect(
                          borderRadius: BorderRadius.circular(
                              12.0), // Apply rounded corners to the container and image
                          child: Container(
                            decoration: BoxDecoration(
                              border: Border.all(
                                  color: Colors.white,
                                  width: 2.0), // Add white border to the images
                              borderRadius: BorderRadius.circular(
                                  12.0), // Ensure rounded corners
                            ),
                            child: ClipRRect(
                              borderRadius: BorderRadius.circular(
                                  10.0), // Apply rounded corners inside the container
                              child: Image.network(
                                images[index],
                                fit: BoxFit
                                ...
                                errorBuilder: (context, error, stackTrace) {
                                  return const Icon(Icons
                                      .broken_image); // Handle error in case of a broken URL
                                },
                              ),
                            ),
                          ),
                      ...
                      crossAxisSpacing:
                          14.0, // Space between tiles horizontally
                    ),
                  ),
      ),
    );
